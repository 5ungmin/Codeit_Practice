...
class CustomerServiceCenter:
    ...
    def process_complaint(self):
        ...
        if len(self.queue) == 0:
            ...
            processing_complaint = self.queue.popleft()
            print(f"{processing_complaint.name}님의 {processing_complaint.content} 문의 내용 접수 되었습니다. "
                  f"담당자가 배정되면 {processing_complaint.email}로 연락드리겠습니다!")
