...
def sublist_max(profits):
    ...
    max_profits = sorted_profits[-1]

    start = profits.index(sorted_profits[-1])
    end = profits.index(sorted_profits[-2])

    max_profits = max(sum(profits[start:end+1]), sum(profits[end:start+1]))
    return max_profits
